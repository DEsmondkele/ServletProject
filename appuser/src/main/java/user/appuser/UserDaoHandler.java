...
public class UserDaoHandler {
    public static Connection connectDB(){
        ...
        try{
            ...
            //it creates a connection to the database using the url
            connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/appuserdb", "adminuser", "user123");
    ...
    public static int addUser(User user) throws SQLException {
       ...
       // SQL statement is precompiled and stored in a PreparedStatement object.
        PreparedStatement preparedStatement = connect.prepareStatement( "insert into user(username,password) values (?,?)");
        //set the parameter to the given Java String value
        preparedStatement.setString(1, user.getUsername());
        preparedStatement.setString(2, user.getPassword());
        //execute SQl statement insert values to the database
    ...
    public static  int updateUser(User user) throws SQLException {
        ...
        Connection connect= UserDaoHandler.connectDB();
        // SQL statement is precompiled and stored in a PreparedStatement object
       PreparedStatement preparedStatement = connect.prepareStatement( "update user set username=?,password=? where id=?");
        //set the parameter to the given Java String value
       preparedStatement.setString(1, user.getUsername());
       preparedStatement.setString(2, user.getPassword());
       //execute SQl statement, insert values to the database
    ...
    public  static  User getUserById(int id) throws SQLException {
        ...
        //set the integer value to the  user id,
        preparedStatement.setInt(1,id);
        //A table of data representing a database result set,generated after the query
        ResultSet resultSet=preparedStatement.executeQuery();
        //checking for saved fields,if more than one
        if(resultSet.next()){
            //value of the column is assigned to the set method
    ...
    public static List<User> getAllUsers(int start, int total) throws SQLException {
        ...
        //create connection at the call of the method
        Connection connect= UserDaoHandler.connectDB();
        //SQL statement and telling it to select from the first index
        PreparedStatement preparedStatement = connect.prepareStatement
                ("select * from user limit "+(start-1)+","+total);
        ResultSet resultSet=preparedStatement.executeQuery();
